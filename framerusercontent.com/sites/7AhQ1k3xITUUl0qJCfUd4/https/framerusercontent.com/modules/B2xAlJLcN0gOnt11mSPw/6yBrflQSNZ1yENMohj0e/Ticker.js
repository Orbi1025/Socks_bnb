 ...
 */ export default function Ticker(props) {
  ...
  const paddingValue = paddingPerSide
    ...
    : `${padding}px`;
  /* Checks */ const currentTarget = RenderTarget.current();
  const isCanvas =
    currentTarget === RenderTarget.canvas ||
    currentTarget === RenderTarget.export; // Remove empty slots (such as hidden layers)
  ...
  /* Arrays */ let clonedChildren = null;
  let dupedChildren = [];
...
const Wrapper = /*#__PURE__*/ forwardRef(({ children, ...props }, ref) => {
  const innerRef = useRef();
  const inView = useInView(innerRef);
  useImperativeHandle(ref, () => innerRef.current);
  useEffect(() => {
    const current = innerRef.current;
    if (!current) return; // for a11y: Manage tabIndex on focusable descendants & aria-hidden on the parent.
    if (inView) {
